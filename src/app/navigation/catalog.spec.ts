...
describe('navigation catalog', () => {
  it('merges domain providers into the current admin navigation tree', () => {
    ...
    expect(findGroup(items, 'student-affairs')?.children.map((item) => item.key)).toEqual([
  ...
  it('preserves current special access rules after moving domain ownership', () => {
    ...
    expect(findGroup(prodAdminItems, 'student-affairs')?.children.map((item) => item.key)).toEqual([
      '/student-affairs/class-adviser-governance',
    ]);
    expect(
  ...
  it('keeps upstream sync pages hidden from student affairs officers', () => {
    ...
    expect(findGroup(staffItems, 'academic-affairs')).toBeUndefined();
  ...
  it('shows academic calendar but keeps course schedule sync admin-only for academic officers', () => {
    ...
    expect(
      canAccessNavigationPath('/student-affairs/class-adviser-governance', {
        accountId: 1002,
        primaryAccessGroup: 'STAFF',
        accessGroup: ['STAFF'],
        slotGroup: ['ACADEMIC_OFFICER'],
  ...
  it('keeps route guard access checks aligned with filtered navigation results', () => {
    ...
    expect(
      canAccessNavigationPath('/class-affairs/student-evaluation-comments', studentFilter),
    ).toBe(false);
    expect(
      canAccessNavigationPath('/class-affairs/student-evaluation-comments', buildFilter()),
    ).toBe(true);
    ...
    expect(
      canAccessNavigationPath('/academic-affairs/class-adviser-governance', buildFilter()),
    ).toBe(false);
    expect(canAccessNavigationPath('/labs/student-private-profile', buildFilter())).toBe(true);
    expect(canAccessNavigationPath('/class-affairs/student-conduct-alignment', buildFilter())).toBe(
      true,
    );
    expect(
      canAccessNavigationPath(
...
describe('student evaluation comment stable entry', () => {
  it.each([
    { accessGroup: ['ADMIN'] as const, slotGroup: [], allowed: true },
    { accessGroup: ['STAFF'] as const, slotGroup: ['CLASS_ADVISER'], allowed: true },
    { accessGroup: ['STAFF'] as const, slotGroup: ['COUNSELOR'], allowed: true },
    { accessGroup: ['STAFF'] as const, slotGroup: ['ACADEMIC_OFFICER'], allowed: false },
    { accessGroup: ['STAFF'] as const, slotGroup: [], allowed: false },
    { accessGroup: ['STUDENT'] as const, slotGroup: ['CLASS_ADVISER'], allowed: false },
  ])(
    'enforces backend entry roles: $accessGroup / $slotGroup',
    ({ accessGroup, slotGroup, allowed }) => {
      for (const appEnv of ['dev', 'test', 'prod'] as const) {
        const filter = buildFilter({
          accessGroup,
          slotGroup,
          primaryAccessGroup: accessGroup[0],
          appEnv,
        });
        expect(canAccessNavigationPath('/class-affairs/student-evaluation-comments', filter)).toBe(
          allowed,
        );
        expect(canAccessNavigationPath('/labs/student-evaluation-comment-workbench', filter)).toBe(
          false,
        );
      }
    },
