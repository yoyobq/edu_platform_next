...
test('忘记密码请求遇到 http transport error 时，应展示服务异常提示', async ({ page }) => {
  await mockForgotPasswordTransportError(page, 'http', 'PASSWORD_RESET_HTTP_FAILED');
...
test('reset intent 查询遇到 http transport error 时，应进入 error 状态并展示服务异常提示', async ({
  page,
}) => {
  await mockResetPasswordIntentTransportError(page, 'http', 'RESET_INTENT_LOOKUP_HTTP_FAILED');

  await page.goto(routes.resetPassword('reset-token-http-failed'));
