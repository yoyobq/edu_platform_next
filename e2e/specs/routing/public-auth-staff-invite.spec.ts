...
test('有效 staff invite 应可完成预览、上游核对与注册消费，且不传 loginEmail', async ({ page }) => {
  ...
  const stepper = page.locator('.ant-steps');
  await expect(stepper.getByText('确认邀请', { exact: true })).toBeVisible();
  await expect(stepper.getByText('身份核对', { exact: true })).toBeVisible();
  await expect(stepper.getByText('设置账户', { exact: true })).toBeVisible();
  await expect(stepper.getByText('完成激活', { exact: true })).toBeVisible();
  await expect(page.getByText('invitee@example.com')).toBeVisible();

  await page.getByRole('button', { name: '下一步：身份核对' }).click();
  await expect(
    page.getByText('这里填写的是学校或上游人事系统账号，不是当前平台登录账号。'),
  ).toBeVisible();
  await page.getByLabel('上游系统账号').fill('teacher.alice');
  await page.getByLabel('上游系统密码').fill('Password!123');
  await page.getByRole('button', { name: '核对身份并进入下一步' }).click();
...
test('上游账号校验失败时应提示明确错误且停留在身份核对阶段', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '核对身份并进入下一步' })).toBeVisible();
});
