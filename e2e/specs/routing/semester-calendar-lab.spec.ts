...
test('学期校历 labs 页应按起止日期所在周渲染并支持查看事件详情', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '进入正式管理页' })).toBeVisible();
  await expect(
  ...
  await expect(page.locator('.ant-tag').filter({ hasText: '教学开始' })).toBeVisible();
  ...
  await page.getByRole('button', { name: '春季运动会' }).click();
...
test('学期校历 labs 页应支持切换学期并刷新周视图内容', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '2026-2027 学年第一学期' }).click();

  await expect(
    page.locator('.ant-card-head-title').filter({ hasText: '2026-2027 学年第一学期' }),
  ).toBeVisible();
  await expect(page.getByText('共 17 周（按教学开始日所在周计算）')).toBeVisible();
