<!-- docs/project-convention/student-conduct-grade-governance.md -->

# 操行数据治理

## 适用范围

- 页面：`/labs/student-conduct-grade-governance`
- 前端实现：`src/labs/student-conduct-grade-governance/`
- 本文描述当前前端对操行治理页面的调用约定、阻断条件和 UI 行为
...
- 显示学期序号徽标
- 表格列：
  `学号 / 姓名 / 分数 / 确认等级 / 本地存储 / 冲突 / 操作`
- `推定等级` 不单列展示，仅作为 `确认等级` tooltip
- `UPSTREAM_CONFIRMED` 不再单独显示为治理状态 tag 文案，列表口径保留“校园网”

## 前端不做的事

- 不按 `studentStatus` 或其他本地字段重算班级名单
- 不按全局 semester 或自然日期自行决定可治理学期
- 不因专业停用状态自行阻断操行页面
- 不自动重复提交写回或清理动作
