...
4. 若已有 token，示例 tab 会按需或自动调用后端读取教师字典、专业字典、班级列表、历史班主任、教职工身份等 upstream 数据
5. 成功后展示后端返回的原始 JSON
...
- 基于当前 upstream 登录用户读取历史班主任班级，并展示 `{ label: name, value: code }` 选择器映射；该数据不是任课班级，也不写入本地班主任任职表
