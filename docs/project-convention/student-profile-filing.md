...
- 提醒列缺失项默认展示为 `缺<资料项>信息`；照片缺失例外，展示为
  `缺照片，请去校园网上传`
- `snapshotPresent === false` 只表示本地资料快照尚未建立；列表仍展示该学生和
  `建档` 操作，但资料进度展示为 `待建档`，提醒列不得展开
  `profileCompletenessFlags` 的逐项缺失。建档成功后才展示 `缺<资料项>信息`
- 当当前班级所有学生均为首次待建档时，表格区域展示空态和 `建档当前班级` 操作，不展示
  一整页待建档行；若存在已建档、需关注或缺学工关联学生，则继续展示列表
- 右侧 `建档概览` 暂不展示；页面以主表格为唯一工作区
