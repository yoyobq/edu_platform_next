...
`/student-affairs/class-adviser-governance` 指派班主任时也遵守该约定：页面优先使用
`staffDirectory` 缓存和 `StaffDirectoryTeacherAutoComplete`，缓存缺失时允许手动输入
`staffId`，并在本地 `member_staff` 尚未建立时向后端提交 `staffName` 快照。
