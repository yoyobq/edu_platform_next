...
后端字段、筛选参数和返回结构，以 [../backend/README.md](../backend/README.md) 指向的后端 schema 来源为准；本文件只收口前端已经落地的页面、分层和展示规则。
...
- 前端只允许以下排序字段：
  ...
  - `loginName`
- 当前筛选控件暴露以下选项：
  - `true`：全部用户类型 / 仅看 `staff`（当前实现都映射为 `hasStaff=true`）
  - `false`：仅看无 `staff`
- 搜索、筛选变化时，列表重置到第一页
...
- 不新增独立“详情”列
- 当前整行点击也会进入详情；状态快捷切换等行内控件必须阻止事件冒泡
- 从列表进入详情页、再返回列表时，应保留当前 search params 上下文
...
1. 页面头部与返回动作
2. 主内容区 `Tabs`
3. `基本资料` tab：账户配置、个人背景
4. `身份信息` tab：基础职务、Staff Slot、staff 参考信息
5. 侧边栏 `快速识别` 卡片
6. 侧边栏 `最近登录` 卡片
