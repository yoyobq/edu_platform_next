...
- `/labs/my-teaching-plan`
  ...
  - 默认选择当前可见学期，按 `scheduleId` 区分同名课程与教学班，并以接近 Excel 的 A–G 列展示教学计划
  - 每条有效 occurrence 固定投影为一行；真源已切分的 `1,2` 与 `3,4` 必须保持两行，不得在前端重组为连续四节
  - 当前 A–G 表格不适用于一体化课程；选中一体化课程时仅展示模板差异提示，不生成填写表格或 Excel 导出
  - 授课方式默认线下；课程安排已有统一授课地点时直接填入 E 列，无值时首次填写会写入后端 `classroomName` 并成为本课程默认地点
  - 单个课次可用本地地点例外覆盖统一地点；表头编辑入口用于整体修改、写回后端并清除当前课程全部逐行例外
  - 授课方式、逐课次地点例外，以及独立成组的 F“授课章节与内容”/G“课外作业”按当前账号、目标教师、学期、课程在当前浏览器保留至最后编辑后 24 小时；统一授课地点不属于限时草稿
  - 页面每次读取当前真源重新生成 A–E；F/G 内容组可手工补行、整组拖动排序和删除，并提供一次删除撤销；内容不足时可拖到任意后方正式课次格，来源与中间位置保留为空位
  - 可在 upstream 登录后从近 6 学期的同教师相似课程中选择一条历史计划；确认后按历史完整行序整组替换 F/G，历史行更多时扩展表格，扩展行的 A–E 保持空白
  - 旧版 `.xls` Excel 文件是用户长期保留完整逐课次教学计划的唯一载体；只有 F/G 内容组数等于正式课次数且正式课次非空时才允许导出，内容单元格本身可为空
  - 停课与调出仅作为低噪音调整记录，不混入有效日期和主指标
  - 普通教师只读取本人计划；管理员、教务人员和教研组长可选择后端授权范围内的教师，最终数据权限以后端为准
  - 当前不发送通知；除课程安排统一授课地点外不写回排课，也不表达实际到课或教学执行结果
