...
| `full` | 240px | 完整占位态；宽屏可默认，显式 pin 后可持久化 |
...
- `GUEST` 账号：当前已启用轻量正式侧栏，可见首页 `/` 与系统管理分组下的异常预览 `/errors/preview`。
...
| `none` → `full`            | 首次投影为 `rail`、无偏好且非极窄竖屏    | layout 默认      |
...
`full`；这次默认展示不会写入 pin 偏好。测试和业务代码不得假设首次进入一定处于 `rail`。
