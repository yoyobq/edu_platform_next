...
- 想确认 upstream token 为什么由前端持有、`/labs/upstream-session-reference` 的最小接入样板、当前 `/labs/upstream-session-demo` 的业务演示边界，以及后续 upstream 功能应复用什么模式时，先看 [project-convention/upstream-session-frontend-ownership.md](./project-convention/upstream-session-frontend-ownership.md)
