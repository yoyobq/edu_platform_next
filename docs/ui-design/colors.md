...
状态色可以参与界面，但应优先以**信号层**出现，而不是以**背景层**出现。
...
- success / warning / processing 这类状态色，默认只进入小载体或极淡边框
- 非阻断场景下，不用粗色条、整块染色、大片色带表达状态
- 同一容器若已有状态点或标签，容器本身的色彩表达应再降一档
- 状态色混入容器时，优先基于 antd token 与 `color-mix()` 做低占比混合，不直接铺纯色
- 红色只服务于错误、风险、异常，不拿来做一般信息分类

判断原则：颜色的职责是帮助扫描，而不是制造第二个标题。
