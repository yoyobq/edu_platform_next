...
edu 后台信息密度高，不需要营销级大标题。页面标题使用 `level={3}`，是在层级感与空间效率之间取平衡；卡片标题使用 `level={5}`，与正文接近、主要靠字重拉开差异，避免卡片内部标题膨胀。
...
| -------------------------- | ----------------------------------------------------------------------------- | -------------------------------- | ------------------- | ----------------------------------------- |
| 页面标题                   | `Typography.Title level={3}`                                                  | 页面最高层级，稳重明确           | 1-2 行              | 用于页面头部；默认页面唯一                |
...
| 标签/元信息                | `Typography.Text type="secondary"`                                            | 时间、状态补充、来源等弱信息     | 1 行                | 不单独发明更小一档正文                    |
...
- antd 默认 `fontSize: 14` 与默认行高继续沿用，不额外自定义
- 默认不手动调整 `leading-*`
...
- 页面最上层标题默认 `Typography.Title level={3}`
