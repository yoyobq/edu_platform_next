...
- 当前代码库中的 `src/app/router/index.tsx` 仍是单根 `/ -> AppLayout -> HomePage` 结构，尚未反映 public / workbench 双分支拆分
- 在开始 P0-1 代码改造前，建议先运行 `npm run lint` 与 `npm run build`，确认当前仓库不存在会干扰路由重构的基础静态错误
- 但这类检查只用于建立实现基线，不替代对“路由治理语义是否已对齐本方案”的代码审查
